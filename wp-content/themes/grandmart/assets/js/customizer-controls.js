...
jQuery( document ).ready(function($) {
    ...
    $(".grandmart-chosen-select").chosen({
        width: "100%"
    });

    // icon picker
    $('.grandmart-icon-picker').each( function() {
    ...
    $( document ).on( 'click', '.customize_multi_add_field', grandmart_customize_multi_add_field )
        .on( 'change', '.customize_multi_single_field', grandmart_customize_multi_single_field )
        .on( 'click', '.customize_multi_remove_field', grandmart_customize_multi_remove_field )
    ...
    function grandmart_customize_multi_add_field(e) {
        ...
        e.preventDefault();
            ...
            grandmart_customize_multi_write( $control );
    }

    function grandmart_customize_multi_single_field() {
        var $control = $( this ).parents( '.customize_multi_input' );
        grandmart_customize_multi_write( $control );
    }

    function grandmart_customize_multi_remove_field(e) {
        e.preventDefault();
        var $this = $( this );
        var $control = $this.parents( '.customize_multi_input' );
        $this.parent().remove();
    ...
    function grandmart_customize_multi_write( $element) {
